		...
		if(!keyword){
			...
			$.ajax({
				...
				success : function(data) {
					...
					ref+= '<th scope="col">환불 날짜</th>';
					ref+= '<th scope="col">결제수단</th>';
					ref+= '</tr></thead>';
					ref+= '<tr>';
					ref+= '<td>';
...
function select_Detail(){
	$('.detail_btn').click(function() {
		...
		var refund_date = $(this).parent().parent().find('td').eq(3).text();
		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#modal_refund_date').text('환불 시각 : ' + refund_date);
...
<div class="modal fade" id="detail_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
	  ...
	  <div style="margin-top: 20px">
	  	<h5 id="modal_refund_date" style="margin-left: 20px"></h5>
	  </div>
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header border-0" style="height : 120px;">
              ...
              <h3 class="mb-0">${store_name} 매장 환불내역</h3>              
              <button type="button" class="btn btn-outline-danger btn-sm" style="margin-left: 20px;" onclick="location.href='${app}/pos/refund/1/'">거래내역 조회 (환불하기)</button>
              </form>
			  ...
			  <button type="button" class="btn btn-warning btn-sm" id="refresh_btn" style="margin-bottom: 25px;" onclick="location.href='${app}/pos/refund/1/list'">
              전체보기
              </button>
  ...
  <nav id="paging_part" aria-label="...">
    ...
    <ul class="pagination justify-content-end mb-0">
      <li class="page-item <c:if test="${pageDTO.pg==1}">disabled</c:if>">
        <a class="page-link" href="${app}/pos/refund/1/list" tabindex="-1">
      ...
      <li class="page-item <c:if test="${pageDTO.startBlock==1}">disabled</c:if>">
        <a class="page-link" href="${app}/pos/refund/${pageDTO.startBlock-1}/list" tabindex="-1">
      ...
      <c:forEach var="p" begin="${pageDTO.startBlock}" end="${pageDTO.endBlock}">
      	<li class="page-item <c:if test="${p==pageDTO.pg}"> active </c:if>">
        	<a <c:if test="${p!=pageDTO.pg}"> href="${app}/pos/refund/${p}/list" </c:if>class="page-link">${p}</a>
      ...
      <li class="page-item <c:if test="${pageDTO.endBlock==pageDTO.totalPage}">disabled</c:if>" >
        <a class="page-link" href="${app}/pos/refund/${pageDTO.endBlock+1}/list">
          <i class="fas fa-angle-right"></i>
        </a>
      </li>
      
      <li class="page-item <c:if test="${pageDTO.pg==pageDTO.totalPage}">disabled</c:if>">
        <a class="page-link" href="${app}/pos/refund/${pageDTO.totalPage}/list" tabindex="-1">
