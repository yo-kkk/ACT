        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header border-0" style="height : 120px;">
			  ...
			  <button type="button" class="btn btn-warning btn-sm" id="refresh_btn" style="margin-bottom: 25px;" onclick="location.href='${app}/pos/stock/1/?keyword='">
              전체보기
              </button>
              </form>
            </div>
...
<div class="card-footer py-4" >
	...
	<select class="form-control" id="other_store" style="width: 300px;">
		<option disabled selected hidden>타매장 선택하기</option>
		<c:forEach items="${store_list}" var="store_list">
			<option value="${store_list}">${store_list}</option>
		</c:forEach>
	</select>
  ...
  <nav aria-label="...">
    ...
    <ul class="pagination justify-content-end mb-0">
      <li class="page-item <c:if test="${pageDTO.pg==1}">disabled</c:if>">
        <a class="page-link" href="${app}/pos/stock/1/?keyword=${keyword}" tabindex="-1">
      ...
      <li class="page-item <c:if test="${pageDTO.startBlock==1}">disabled</c:if>">
        <a class="page-link" href="${app}/pos/stock/${pageDTO.startBlock-1}/?keyword=${keyword}" tabindex="-1">
      ...
      <c:forEach var="p" begin="${pageDTO.startBlock}" end="${pageDTO.endBlock}">
      	<li class="page-item <c:if test="${p==pageDTO.pg}"> active </c:if>">
        	<a <c:if test="${p!=pageDTO.pg}"> href="${app}/pos/stock/${p}/?keyword=${keyword}" </c:if>class="page-link">${p}</a>
      ...
      <li class="page-item <c:if test="${pageDTO.endBlock==pageDTO.totalPage}">disabled</c:if>" >
        <a class="page-link" href="${app}/pos/stock/${pageDTO.endBlock+1}/?keyword=${keyword}">
