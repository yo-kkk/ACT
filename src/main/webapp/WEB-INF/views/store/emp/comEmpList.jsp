...
$(document).ready(function(){
	...
	$('#search_btn').click(function() {
		...
		location.href='${app}/store/emp/empList/1/?keyword='+keyword;
	});
	
	$('#refresh_btn').click(function() {	
		location.href='${app}/store/emp/empList/1/?keyword=';
	});
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header border-0">
              <h3 class="mb-0">${store_name} 직원 목록</h3>
                  <!-- 검색창 -->         
  ...
  <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 900px;">
    <div class="modal-content">
      ...
      <div class="modal-body">
       <!-- update part start -->      
		...
		<form id="empUpdateForm"  class="table-responsive" action="empUpdate" method="post">
			...
			<tr>
				<th scope="col">지점명</th>			
				<td><input disabled id="storeName"   class="form-control"  type="text" name="storeDTO.store_name" autofocus required/></td>
			</tr>
			<tr>
				<th scope="col">이름</th>
			...
			<tr>
				<th scope="col">직급</th>
				<td><input disabled id="posit_name"  class="form-control"   type="text" name="positionDTO.name" required/></td>
			</tr>
			<tr>
				<th scope="col">기본급</th>
      ...
      <div class="modal-footer" style="padding: 0px;">
        <input class="form-control" id="retireDate"   name="retireDate" type="date" required/>
  ...
  <nav aria-label="..." style="margin: 20px;">
    ...
    <ul class="pagination" style="justify-content: center;">
      <li class="page-item <c:if test="${pageDTO.pg==1}">disabled</c:if>">
        <a class="page-link" href="${app}/store/emp/empList/1/?keyword=${keyword}" tabindex="-1">
      ...
      <li class="page-item <c:if test="${pageDTO.startBlock==1}">disabled</c:if>">
        <a class="page-link" href="${app}/store/emp/empList/${pageDTO.startBlock-1}/?keyword=${keyword}" tabindex="-1">
          <i class="fas fa-angle-left"></i>
        </a>
      </li>     
      <c:forEach var="p" begin="${pageDTO.startBlock}" end="${pageDTO.endBlock}">
      	<li class="page-item <c:if test="${p==pageDTO.pg}"> active </c:if>">
        	<a <c:if test="${p!=pageDTO.pg}"> href="${app}/store/emp/empList/${p}/?keyword=${keyword}" </c:if>class="page-link">${p}</a>
      ...
      <li class="page-item <c:if test="${pageDTO.endBlock==pageDTO.totalPage}">disabled</c:if>" >
        <a class="page-link" href="${app}/store/emp/empList/${pageDTO.endBlock+1}/?keyword=${keyword}">
          <i class="fas fa-angle-right"></i>
        </a>
      </li>
      
      <li class="page-item <c:if test="${pageDTO.pg==pageDTO.totalPage}">disabled</c:if>">
        <a class="page-link" href="${app}/store/emp/empList/${pageDTO.totalPage}/?keyword=${keyword}" tabindex="-1">
