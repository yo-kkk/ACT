...
$(document).ready(function() {
	...
	$('.receiveBtn').click(function(){
		...
		var store_order_code = $(this).parent().parent().find('td').eq(0).text().trim();
		var product_code = $(this).parent().parent().find('td').eq(3).text().trim();
		var quantity = $(this).parent().parent().find('td').eq(4).text().trim();
		...
		if(receive_yn == "n"){
			if(confirm("해당 발주건을 수령 처리하시겠습니까?")){
				 ...
				 frm.appendChild(addData("storeDTO.store_name",store_name));
				 $('#fromStoreToHeadbody').append(frm);
				 frm.submit();
			}	
...
<div id="fromStoreToHeadbody">
<input type="hidden" value="${store_name}" name="store_name" id="store_name"/>
      ...
      <div class="col">
          <div class="card shadow">
            <div class="card-header border-0">
              <h2 class="mb-0">${store_name} 발주 신청 내역</h2>
            </div>
            ...
            <div class="table-responsive">
              <table class="table align-items-center table-flush">            
                ...
                <tr class="toHeadOrder_list btn-outline-default" >              
                    <td>
                    ...
                    <td>
	                   <c:if test="${list.send_yn eq 'n'}">  
	                     <button type="button" class="btn btn-danger" >미발송</button>                     
	                    </c:if>
	                    <c:if test="${list.send_yn eq 'y'}">  
	                     <button type="button" class="btn btn-success" >배송중</button>                     
	                    </c:if>
					</td>
					<td>
					 <c:if test="${list.send_yn eq 'n'}">   
					 ...
					 <c:if test="${list.send_yn eq 'n'}">   
						...
						<fmt:formatDate value="${orderDate01}" pattern="yyyy.MM.dd" var="orderDate02" />
                       	<c:if test="${orderDate02 ne nowDate}">
							<button type="button" value=" ${list.order_date}" class="btn btn-primary" disabled>X</button>
						</c:if> <c:if test="${orderDate02 eq nowDate}">
  ...
  <nav aria-label="..." style="margin: 20px;">
    ...
    <ul class="pagination" style="justify-content: center;">
      <li class="page-item <c:if test="${pageDTO.pg==1}">disabled</c:if>">
        <a class="page-link" href="${app}/store/order/1/?keyword=${keyword}" tabindex="-1">
      ...
      <li class="page-item <c:if test="${pageDTO.startBlock==1}">disabled</c:if>">
        <a class="page-link" href="${app}/store/order/${pageDTO.startBlock-1}/?keyword=${keyword}" tabindex="-1">
      ...
      <c:forEach var="p" begin="${pageDTO.startBlock}" end="${pageDTO.endBlock}">
      	<li class="page-item <c:if test="${p==pageDTO.pg}"> active </c:if>">
        	<a <c:if test="${p!=pageDTO.pg}"> href="${app}/store/order/${p}/?keyword=${keyword}" </c:if>class="page-link">${p}</a>
      ...
      <li class="page-item <c:if test="${pageDTO.endBlock==pageDTO.totalPage}">disabled</c:if>" >
        <a class="page-link" href="${app}/store/order/${pageDTO.endBlock+1}/?keyword=${keyword}">
